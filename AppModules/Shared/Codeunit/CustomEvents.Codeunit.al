codeunit 50100 "Custom Events N24"
{
    ...
    local procedure TransferOrderPostReceipt_OnAfterInsertTransRcptHeader(var TransferReceiptHeader: Record "Transfer Receipt Header"; TransferHeader: Record "Transfer Header")
    begin
        TransferReceiptHeader.CopyFieldsFromTransferHeader(TransferReceiptHeader, TransferHeader);
    end;

    [EventSubscriber(ObjectType::Table, Database::"Transfer Shipment Header", OnAfterCopyFromTransferHeader, '', false, false)]
    local procedure TransferOrderPostShipment_OnAfterInsertTransShptHeader(var TransferShipmentHeader: Record "Transfer Shipment Header"; TransferHeader: Record "Transfer Header")
    begin
        TransferShipmentHeader.CopyFieldsFromTransferHeader(TransferHeader, TransferShipmentHeader);
    end;
