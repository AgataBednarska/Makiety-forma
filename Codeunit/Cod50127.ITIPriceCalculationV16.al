codeunit 50127 "ITI Price Calculation - V16" implements "Price Calculation"
{
    trigger OnRun()
    var
    ...
    var
        ...
        GetPriceLineDCY: Page "ITI Get Price Line DCY";
        Line: Variant;
    ...
    begin
        ...
        if FindLines(AmountType, TempPriceListLine, PriceCalculationBufferMgt, ShowAll) then begin
            ...
            GetPriceLineDCY.SetDataCaptionExpr(PriceAssetList);
            GetPriceLineDCY.SetForLookup(CurrLineWithPrice, AmountType, TempPriceListLine);
            CurrLineWithPrice.GetLine(Line);
            SalesLine := Line;
            GetPriceLineDCY.SetCurrency(SalesLine."Currency Code", SalesLine."Posting Date");
            if GetPriceLineDCY.RunModal() = ACTION::LookupOK then begin
                GetPriceLineDCY.GetRecord(TempPriceListLine);
                TempPriceListLine."Currency Code" := '';
                TempPriceListLine."Unit Price" := GetPriceLineDCY.GetUnitPriceDCY();
